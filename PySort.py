# 
# This program is a simple sort program just made with the intent of making life a bit easier for people with messy
# desktops mainly.
...
from tkinter import filedialog, Button, Tk, Label, messagebox, Listbox, Scrollbar
...
def identify():
    for mn in glob('*'):
        ...
        # Executables
        if new == "('application/octet-stream', None)" \
                or glob(mn) == glob("*.msi"):
            Create(mn, "Executables")
            Executables.append(mn)

        # Videos
        elif new == "('video/x-msvideo', None)" \
                or new == "('video/mp4', None)" \
                or glob(mn) == glob('*.webm') \
                or glob(mn) == glob('*.flv'):
            Create(mn, "Video")
            Videos.append(mn)

        # Images
        elif new == "('image/x-png', None)" \
                or new == "('image/pjpeg', None)" \
                or new == "('image/gif', None)" \
                or new == "('image/jpeg', None)":
            Create(mn, "Images")
            Images.append(mn)

        # Audio Basic
        elif new == "('audio/x-mpg', None)" \
                or new == "('audio/x-wav', None)" \
                or new == "('audio/x-ms-wma', None)" \
                or new == "('midi/mid', None)":
            Create(mn, "Audio")
            Audios.append(mn)

        # Documents
        elif new == "('application/vnd.openxmlformats-officedocument.wordprocessingml.document', None)" \
                or new == "('application/x-msexcel', None)" \
                or new == "('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', None)" \
                or new == "('application/x-mspowerpoint', None)" or new == "('application/x-mspowerpoint.12', None)" \
                or new == "('application/msword', None)" \
                or new == "('application/pdf', None)":
            Create(mn, "Documents")
            Docs.append(mn)
        ...
        # Compressed
        elif new == "('application/x-tar', 'gzip')" \
                or new == "('application/zip', None)" \
                or glob(mn) == glob(
                '*.rar') or glob(mn) == glob('*.7z'):
    ...
    if len(Errors) > 0:
        messagebox.showinfo("ERRORS OCCURED!", (str(len(
            Errors))) + " file(s) were not moved.")  # file(s) were not moved because they are already in the
                                                     #  destination folders:\n"+'\n'.join(Errors))
    return "Complete!"
...
lab = Label(root,
            text="Welcome to PySort! An application from a lazy guy to all lazy people!\nYou are currently in: %s"
                 % str(getcwd()))
# ^ This creates a label on the main windows(root) which just prints text.) ^

lab.pack()  # The pack feature needs to be used on any object created on the root windows.
            #  This is to tell python to place the object on the screen.
...
def butt():
    ...
    newdir = filedialog.askdirectory()  # This  is a method in filedialog(class in tkinter) that makes a folder
                                        #  selection screen.
    if newdir == "":
        pass
    else:
        ...
        try:
            if newdir == (b + ":/Windows/") or newdir[3:9] == notallowed[3:9]:
                messagebox.showerror("PySort",
                                     "Do not select that directory!")  # Messagebox prints a popup message on the
                                                                       # screen, depending on the method (in this
                                                                       # case, showerror.)
            else:
                chdir(newdir)
                lab.config(
                    text="Welcome to PySort! An application from a lazy guy to all lazy people!\nYou are "
                         "currently in: %s" % str(getcwd()))
...
def ok():
    ...
    origdir = str(getcwd())
    listbox.delete(1, "end")
    del (Executables[:], Audios[:], Videos[:], Images[:], Docs[:], HTML[:], Compress[:], Text[:], PyScripts[:],
         Nokia[:], Torrent[:], ISO[:], Short[:], Un[:])
    mainlist = [Executables, Audios, Videos, Images, Docs, HTML, Compress, Text, PyScripts, Nokia, Torrent, ISO, Short,
                Un]
    # bar = progress()
